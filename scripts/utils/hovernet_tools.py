...
def make_auto_mask(slide: OpenSlide, mask_level: int, save: Optional[str] = None) -> ndarray:
    ...
    desired_dims = (slide.level_dimensions[0][1] // (2 ** mask_level), 
...
def extract_tiles_hovernet(image_path, json_path, level = 0, size = (64, 64), dict_types = None, save_images = None): #must adapt for segmentation without classification
    """
    ...
    for i in tqdm(range(len(cell_table))):
        ...
        img_cell = slide.read_region((int(cell_line["x"].values[0])-size[0]//2, int(cell_line["y"].values[0])-size[1]//2), level, size)
