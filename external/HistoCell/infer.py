...
def main():
    ...
    val_data = TileBatchDataset(
        ...
        aug=False,
        val=args.omit_gt,
        panuke=args.val_panuke,
        max_cell_num=config.data.max_cell_num,
    )
...
def val_loop(epoch, model, val_loader, device):
    ...
    for idx, data in enumerate(val_bar):
        ...
        prob_list, pred_proportion, tissue_cat, cell_embeddings = model(
        ...
        for name, pb, pp, vix, cc, ct, cell_features in zip(
            name_list, prob_list, pred_proportion, valid_list, coord_list, type_list, cell_embeddings
        ):
