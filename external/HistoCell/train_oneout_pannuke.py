...
def train_loop(epoch, model, train_loader, loss_func, aux_loss, opt, device):
    model.train()
    train_bar = tqdm(
        train_loader, desc="epoch " + str(epoch), total=len(train_loader), unit="batch", dynamic_ncols=True
    )
    for idx, data in enumerate(train_bar):
        ...
        loss_out = loss_func((cell_proportion + 1e-10).log(), pred_proportion + 1e-10) + loss_func(
            (pred_proportion + 1e-10).log(), cell_proportion + 1e-10
        )
        loss_out.backward()
        opt.step()
        opt.zero_grad()
...
def val_loop(epoch, model, val_loader, device):
    model.train()
    val_bar = tqdm(val_loader, desc="epoch " + str(epoch), total=len(val_loader), unit="batch", dynamic_ncols=True)
    ...
    for idx, data in enumerate(val_bar):
        ...
        batch, cells, channels, height, width = images.shape
        images = images.reshape(batch * cells, channels, height, width)
        prob_list, pred_proportion, _, cell_embeddings = model(
            tissue, images, adj_mat, cell_size, valid_mask, {"batch": batch, "cells": cells}
        )
        name_list, valid_list, coord_list, type_list, cell_prop_list = [], [], [], [], []
        for name, single_prop, valid_index, valid_coord, valid_type in zip(
            data["name"], cell_proportion, valid_mask, cell_coords, ref_type
        ):
            if valid_index <= 0:
                continue
        ...
        for name, pb, pp, cp, vix, cc, ct in zip(
            name_list, prob_list, pred_proportion, cell_prop_list, valid_list, coord_list, type_list
        ):
...
def main():
    ...
    for idx, val_prefix in enumerate(data_prefix):
        ...
        raw_prefix.pop(idx)
        val_fold = TileBatchDataset(
            config.data.tile_dir,
            config.data.mask_dir,
            config.data.tissue_dir,
            config.data.cell_dir,
            deconv=config.data.deconv,
            prefix=[val_prefix],
            focus_tissue=args.tissue,
        )
        train_fold = TileBatchDataset(
            config.data.tile_dir,
            config.data.mask_dir,
            config.data.tissue_dir,
            config.data.cell_dir,
        ...
        train_fold, _ = torch.utils.data.random_split(
            train_fold,
            [int(len(train_fold) * args.sample_ratio), len(train_fold) - int(len(train_fold) * args.sample_ratio)],
        )
        ...
        model_dir, ckpt_dir = os.path.join(config.data.save_model, args.model, val_prefix), os.path.join(
            config.data.ckpt, args.model, val_prefix
        )
