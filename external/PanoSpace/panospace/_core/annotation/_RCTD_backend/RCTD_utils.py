...
def get_der_fast(S, B, S_mat, gene_list, prediction, bulk_mode=False, likelihood_vars=None):
    ...
    hess = (-np.asarray(d2_vec)[:, None, None] * S_mat).sum(0)
...
def solveWLS(S, B, S_mat, initialSol, nUMI, bulk_mode=False, constrain=False, likelihood_vars=None, solver="osqp"):
    ...
    alpha = 0.3
    if constrain:
        solution = solution + alpha * solve_qp(
            np.array(D_mat),
            -np.array(d_vec),
            -np.array(A),
            -np.array(bzero),
            np.ones(solution.shape[0]),
            1 - solution.sum(),
            solver=solver,
        )
    else:
        solution = solution + alpha * solve_qp(
            np.array(D_mat), -np.array(d_vec), -np.array(A), -np.array(bzero), solver=solver
        )
    return solution


def solveIRWLS_weights(
    S,
    B,
    nUMI,
    OLS=False,
...
def fitBulk(RCTD, loggings=None):
    bulkData = prepareBulkData(
        RCTD["cell_type_info"]["info"]["cell_type_means"], RCTD["spatialRNA"], RCTD["internal_vars"]["gene_list_bulk"]
    )
    loggings.info("fitBulk: decomposing bulk")
    decompose_results = decompose_full(
        bulkData["X"],
        RCTD["spatialRNA"]["nUMI"].sum().item(),
        np.array(bulkData["b"]),
        verbose=False,
        constrain=False,
        MIN_CHANGE=RCTD["config"]["MIN_CHANGE_BULK"],
        n_iter=100,
        bulk_mode=True,
        loggings=loggings,
    )
    ...
    RCTD["cell_type_info"]["renorm"]["cell_type_means"] = get_norm_ref(
        RCTD["spatialRNA"],
        RCTD["cell_type_info"]["info"]["cell_type_means"],
        RCTD["internal_vars"]["gene_list_bulk"],
...
def SpatialRNA(coords, counts, nUMI=None):
    ...
    if len(barcodes) == 0:
        raise ValueError(
            "SpatialRNA: coords, counts, and nUMI do not share any barcode names. Please ensure that rownames(coords) matches colnames(counts) and names(nUMI)"
        )
...
def Reference(counts, cell_types, nUMI=None, n_max_cells=10000, loggings=None):
    ...
    if cur_count > n_max_cells:
        loggings.info(
            "Reference: number of cells per cell type is {}, larger than maximum allowable of {}. Downsampling number of cells to: {}.".format(
                cur_count, n_max_cells, n_max_cells
            )
        )
...
def process_cell_type_info(reference, cell_type_names, CELL_MIN=25, loggings=None):
    ...
    if reference["cell_types"].value_counts().min() < CELL_MIN:
        loggings.info(
            "process_cell_type_info error: need a minimum of {} cells for each cell type in the reference".format(
                CELL_MIN
            )
        )
    cell_type_info = get_cell_type_info(
        reference["counts"], reference["cell_types"], reference["nUMI"], cell_type_names=cell_type_names
    )
...
def get_de_genes(cell_type_info, puck, fc_thresh=1.25, expr_thresh=0.00015, MIN_OBS=3, loggings=None):
    total_gene_list = []
    epsilon = 1e-9
    ...
    for cell_type in cell_type_info["cell_type_names"]:
        other_mean = (
            cell_type_info["cell_type_means"].loc[gene_list, cell_type_info["cell_type_names"] != cell_type].mean(1)
        )
        logFC = np.log(cell_type_info["cell_type_means"].loc[gene_list, cell_type] + epsilon) - np.log(
            other_mean + epsilon
        )
        type_gene_list = np.where(
            ((logFC > fc_thresh) & (cell_type_info["cell_type_means"].loc[gene_list, cell_type] > expr_thresh)) == True
        )[0]
...
def create_RCTD(
    ...
    if cell_type_info is None:
        cell_type_info = {
            "info": process_cell_type_info(
                reference, cell_type_names=cell_type_names, CELL_MIN=CELL_MIN_INSTANCE, loggings=loggings
            ),
            "renorm": None,
        }
    if not keep_reference:
        reference = create_downsampled_data(reference, n_samples=5)
    puck_original = restrict_counts(
        spatialRNA, np.array(spatialRNA["counts"].index), UMI_thresh=config["UMI_min"], UMI_max=config["UMI_max"]
    )
    ...
    if class_df is None:
        class_df = pd.DataFrame(
            cell_type_info["info"]["cell_type_names"],
            index=cell_type_info["info"]["cell_type_names"],
            columns=["class"],
        )
    internal_vars = {
    ...
    RCTD = {
        "spatialRNA": puck,
        "originalSpatialRNA": puck_original,
        "reference": reference,
        "config": config,
...
def choose_sigma_c(RCTD, Q_mat_all, X_vals_loc, loggings=None):
    ...
    if N_fit == 0:
        raise ValueError(
            "choose_sigma_c determined a N_fit of 0! This is probably due to unusually low UMI counts per bead in your dataset. Try decreasing the parameter UMI_min_sigma. It currently is {} but none of the beads had counts larger than that.".format(
                MIN_UMI
            )
        )
    ...
    for _ in np.arange(RCTD["config"]["N_epoch"]):
        ...
        results = decompose_batch(
            np.array(puck["nUMI"].loc[fit_ind]).squeeze(),
            RCTD["cell_type_info"]["renorm"]["cell_type_means"],
            beads,
        ...
        prediction = (
            RCTD["cell_type_info"]["renorm"]["cell_type_means"].loc[RCTD["internal_vars"]["gene_list_reg"], :]
            @ weights.T
            * (puck["nUMI"].loc[fit_ind]).values.squeeze()[None, :]
        )
        logger.debug(
            "Likelihood value: {}".format(
...
def fitPixels(RCTD, doublet_mode="full", loggings=None):
    RCTD["internal_vars"]["cell_types_assigned"] = True
    likelihood_vars = {
        "Q_mat": RCTD["internal_vars"]["Q_mat"],
        "X_vals": RCTD["internal_vars"]["X_vals"],
        "N_X": RCTD["internal_vars"]["Q_mat"].shape[1],
        "K_val": RCTD["internal_vars"]["Q_mat"].shape[0] - 3,
    }
    cell_type_info = RCTD["cell_type_info"]["renorm"]
    ...
    if doublet_mode == "doublet":
        results = process_beads_batch(
            ...
            constrain=False,
            MAX_CORES=RCTD["config"]["max_cores"],
            MIN_CHANGE=RCTD["config"]["MIN_CHANGE_REG"],
            loggings=loggings,
            likelihood_vars=likelihood_vars,
        )
        ...
        beads = RCTD["spatialRNA"]["counts"].loc[RCTD["internal_vars"]["gene_list_reg"], :].values.T
        results = decompose_batch(
            np.array(RCTD["spatialRNA"]["nUMI"]).squeeze(),
            cell_type_info["cell_type_means"],
            beads,
            RCTD["internal_vars"]["gene_list_reg"],
            constrain=False,
            max_cores=RCTD["config"]["max_cores"],
        ...
        RCTD["results"] = pd.DataFrame(
            weights,
            index=RCTD["spatialRNA"]["counts"].columns,
            columns=RCTD["cell_type_info"]["renorm"]["cell_type_names"],
        )
        return RCTD
