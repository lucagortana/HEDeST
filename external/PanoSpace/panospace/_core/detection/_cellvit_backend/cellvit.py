# -*- coding: utf-8 -*-
# CellViT network for nuclei/cell segmentation
#
...
class CellViT(nn.Module):
    ...
    def calculate_instance_map(
        ...
        # print(predictions_["nuclei_type_map"].shape)
        ...
        cell_post_processor = DetectionCellPostProcessor(nr_types=self.num_nuclei_classes)
        instance_preds = []
        type_preds = []

        for i in range(predictions_["nuclei_binary_map"].shape[0]):
            # print(predictions_["nuclei_type_map"].shape)
            # print(torch.argmax(predictions_["nuclei_type_map"], dim=-1)[i].detach().cpu()[..., None].shape)
            # print(torch.argmax(predictions_["nuclei_binary_map"], dim=-1)[i].detach().cpu()[..., None].shape)
            # print(predictions_["hv_map"][i].detach().cpu().shape)
...
# -----------------------------------------------------------------------------
# CellViT256 : HIPT ViT-256 variant
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
from functools import partial
from .backbones import ViTCellViTDeit
