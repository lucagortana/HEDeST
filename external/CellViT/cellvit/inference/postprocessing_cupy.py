...
class DetectionCellPostProcessor:
    ...
    def post_process_single_image(self, pred_map: cp.ndarray) -> Tuple[np.ndarray, dict[int, dict]]:
    ...
    def _stack_pred_maps(
        ...
        assert isinstance(nuclei_type_map, cp.ndarray), "nuclei_type_map must be a cupy array"
        assert isinstance(nuclei_binary_map, cp.ndarray), "nuclei_binary_map must be a cupy array"
    ...
    def _proc_np_hv(self, pred_inst: cp.ndarray, object_size: int = 10, ksize: int = 21) -> np.ndarray:
