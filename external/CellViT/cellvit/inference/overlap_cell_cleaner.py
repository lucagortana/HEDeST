...
class OverlapCellCleaner:
    def __init__(self, cell_list: List[dict], logger: logging.Logger) -> None:
        ...
        self.mid_cells = self.cell_df[self.cell_df["cell_status"] == 0]  # cells in the mid
    ...
    def _test_cell_list(self, cell_list: List[dict]):
        ...
        for key in required_keys:
            assert key in cell_list[0], f"Key {key} not found in the first dictionary of cell_list"
    ...
    def _clean_edge_cells(self) -> pd.DataFrame:
        ...
        edge_cells_unique[list(type_mapping.keys())] = edge_cells_unique[list(type_mapping.keys())].astype(type_mapping)
    ...
    def _remove_overlap(self, cleaned_edge_cells: pd.DataFrame) -> pd.DataFrame:
        ...
        if not shapely_ge_2_0:
            merged_cells = self._remove_overlap_shapely_1_8(cleaned_edge_cells=cleaned_edge_cells)
        else:
            merged_cells = self._remove_overlap_shapely_2(cleaned_edge_cells=cleaned_edge_cells)
        return merged_cells

    def _remove_overlap_shapely_1_8(self, cleaned_edge_cells: pd.DataFrame) -> pd.DataFrame:
        """Remove overlapping cells from provided DataFrame
        ...
        for iteration in range(20):
            ...
            for query_poly in poly_list:
                if query_poly.uid not in iterated_cells:
                    intersected_polygons = tree.query(query_poly)  # this also contains a self-intersection
    ...
    def _remove_overlap_shapely_2(self, cleaned_edge_cells: pd.DataFrame) -> pd.DataFrame:
        """Remove overlapping cells from provided DataFrame
        ...
        for iteration in range(20):
            ...
            for i, (query_poly, orig_idx) in enumerate(poly_with_idx):
                if orig_idx not in iterated_cells:
                    ...
                    if len(intersected_indices) > 1:  # we have at least one intersection with another cell
                        submergers = []  # all cells that overlap with query
                        for inter_idx in intersected_indices:
                            inter_poly, inter_orig_idx = poly_with_idx[inter_idx]

                            if inter_orig_idx != orig_idx and inter_orig_idx not in iterated_cells:
                        ...
                        if len(submergers) == 0:
                            ...
                            selected_poly_index = np.argmax(np.array([p.area for p, _ in submergers]))
    ...
    def convert_coordinates_vectorized(self, cell_df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        cell_df[["patch_row", "patch_col"]] = pd.DataFrame(cell_df["patch_coordinates"].tolist(), index=cell_df.index)
        cell_df["patch_coordinates"] = cell_df["patch_coordinates"].apply(lambda x: "_".join(map(str, x)))
