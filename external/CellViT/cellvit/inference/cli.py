...
class InferenceConfiguration:
    ...
    def __set_nuclei_taxonomy(self, config: dict) -> None:
        ...
        if "nuclei_taxonomy" in config:
            if config["nuclei_taxonomy"] is not None:
                ...
                assert isinstance(config["nuclei_taxonomy"], str), "Nuclei taxonomy must be of type string"
    ...
    def __set_gpu(self, config: dict) -> None:
        ...
        if gpu is not None:
            ...
            assert 0 <= gpu < torch.cuda.device_count(), f"GPU must be between 0 and {torch.cuda.device_count()-1}"
            self.gpu = gpu
    ...
    def __set_outdir(self, config: dict) -> None:
        ...
        assert isinstance(output_format["outdir"], str), "Output directory must be of type string"
    ...
    def __set_geojson(self, config: dict) -> None:
        ...
        if "geojson" in output_format and output_format["geojson"] is not None:
            assert isinstance(output_format["geojson"], bool), "Geojson must be of type boolean"
    ...
    def __set_graph(self, config: dict) -> None:
        ...
        if "graph" in output_format and output_format["graph"] is not None:
            assert isinstance(output_format["graph"], bool), "Graph must be of type boolean"
    ...
    def __set_compression(self, config: dict) -> None:
        ...
        if "compression" in output_format and output_format["compression"] is not None:
            assert isinstance(output_format["compression"], bool), "Compression must be of type boolean"
    ...
    def __set_ray_remote_cpus(self, config: dict) -> None:
        ...
        if ray_remote_cpus is not None:
            assert isinstance(ray_remote_cpus, int), "Ray remote CPUs must be of type integer"
    ...
    def __set_command(self, config: dict) -> None:
        ...
        """
        assert "process_wsi" in config or "process_dataset" in config, "Command must be provided"
        if "process_wsi" in config:
            assert isinstance(config["process_wsi"], dict), "Command must be of type dict"
            self.command = "process_wsi"

            assert "wsi_path" in config["process_wsi"], "WSI path must be provided"
            assert isinstance(config["process_wsi"]["wsi_path"], str), "WSI path must be of type string"
            assert Path(config["process_wsi"]["wsi_path"]).exists(), "WSI path does not exist"
            if not Path(config["process_wsi"]["wsi_path"]).is_file():
                wsi_path = Path(config["process_wsi"]["wsi_path"])
                files_sorted = sorted(wsi_path.glob("*.dcm"), key=lambda f: f.stat().st_size, reverse=True)
        ...
        if "process_dataset" in config:
            assert isinstance(config["process_dataset"], dict), "Command must be of type dict"
            ...
            assert ("wsi_folder" in process_dataset and process_dataset["wsi_folder"] is not None) ^ (
                "wsi_filelist" in process_dataset and process_dataset["wsi_filelist"] is not None
            ), "Either 'wsi_folder' or 'wsi_filelist' must be provided, but not both."
            if "wsi_folder" in process_dataset and process_dataset["wsi_folder"] is not None:
                assert isinstance(process_dataset["wsi_folder"], str), "WSI folder must be of type string"
                assert Path(process_dataset["wsi_folder"]).exists(), "WSI folder does not exist"
                assert Path(process_dataset["wsi_folder"]).is_dir(), "WSI folder is not a directory"
                ...
                if wsi_extension is not None:
                    assert isinstance(wsi_extension, str), "WSI extension must be of type string"
                    self.wsi_extension = wsi_extension
            else:
                assert isinstance(process_dataset["wsi_filelist"], str), "WSI filelist must be of type string"
                assert Path(process_dataset["wsi_filelist"]).exists(), "WSI filelist does not exist"
                assert Path(process_dataset["wsi_filelist"]).is_file(), "WSI filelist is not a file"
                assert Path(process_dataset["wsi_filelist"]).suffix in [".csv"], "WSI filelist must be a .csv file"
                ...
                assert "path" in filelist_header, "Filelist must contain a 'path' column"
...
class InferenceWSIParser:
    ...
    def __init__(self) -> None:
        ...
        inference_group.add_argument("--gpu", type=int, default=0, help="GPU ID to use for inference")
        ...
        wsi_parser = mode_subparsers.add_parser("process_wsi", help="Process a single Whole Slide Image")
        ...
        # Dataset processing
        dataset_parser = mode_subparsers.add_parser("process_dataset", help="Process multiple WSI files")
        ...
        dataset_parser.add_argument("--wsi_mpp", type=float, help="Microns per pixel (spatial resolution)")
        dataset_parser.add_argument("--wsi_magnification", type=int, help="Magnification level of the slides")

        # System Settings
        system_group = parser.add_argument_group("System Settings")
        system_group.add_argument("--cpu_count", type=int, help="Number of CPU cores to use for inference")
        system_group.add_argument(
            "--ray_worker",
            type=int,
            help="Number of ray worker to use for inference (limited by cpu-count)",
        )
        system_group.add_argument("--ray_remote_cpus", type=int, help="Number of CPUs per ray worker")
    ...
    def _transform_cli_to_yaml_structure(self, opt) -> dict:
        ...
        if opt["mode"] == "process_wsi":
            ...
            opt_yaml_style["process_wsi"]["wsi_magnification"] = opt["wsi_magnification"]
            ...
            opt_yaml_style["process_dataset"]["wsi_magnification"] = opt["wsi_magnification"]
        else:
            raise NotImplementedError("Problem occured - use either process_wsi or process_dataset")
...
def get_user_input_with_timeout(prompt: str, timeout: int = 5, default: bool = True) -> bool:
