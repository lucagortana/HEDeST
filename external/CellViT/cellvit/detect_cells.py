...
from cellvit.inference.cli import get_user_input_with_timeout
...
def main():
    ...
    if command.lower() == "process_wsi":
        ...
        if args["wsi_filelist"] is not None:
            ...
            args["wsi_filelist"]["path"] = args["wsi_filelist"]["path"].apply(lambda x: Path(x))
            args["wsi_filelist"]["filename"] = args["wsi_filelist"]["path"].apply(lambda x: x.name)
            wsi_index_keep = []
            for wsi_index, wsi_file in enumerate(args["wsi_filelist"]["path"]):
                ...
                if wsi_file.is_dir():
                    ...
                    if len(files_sorted) != 0:
                        args["wsi_filelist"].at[wsi_index, "path"] = str(files_sorted[0])  # Assign the largest file
                        wsi_index_keep.append(wsi_index)
                else:
                    if wsi_file.exists():
                        wsi_index_keep.append(wsi_index)
            args["wsi_filelist"] = args["wsi_filelist"].loc[wsi_index_keep].reset_index(drop=True)
            ...
            if (celldetector.outdir / "processed_files.json").exists():
                ...
                if len(processed_files) != 0:
                    celldetector.logger.info(f"Found processed files: {len(processed_files)}")
                    ...
                    if remove_files:
                        ...
                        celldetector.logger.info(f"New processing amount: {len(args['wsi_filelist'])}")

            for wsi_index in range(len(args["wsi_filelist"])):
                celldetector.logger.info(f"Progress: {wsi_index+1}/{len(args['wsi_filelist'])}")
                ...
                if "wsi_magnification" in column_names:
                    wsi_magnification = args["wsi_filelist"].iloc[wsi_index]["wsi_magnification"]
            ...
            celldetector.logger.info(f"Loading all files from folder {args['wsi_folder']}. No filelist provided.")
            if args["wsi_extension"].lower() == "dcm":
                wsi_subfolder = [f for f in sorted(Path(args["wsi_folder"]).iterdir()) if f.is_dir()]
                cleaned_folders = []
                ...
                wsi_filelist = [f for f in sorted(Path(args["wsi_folder"]).glob(f"**/*.{args['wsi_extension']}"))]
            ...
            if (celldetector.outdir / "processed_files.json").exists():
                ...
                if len(processed_files) != 0:
                    ...
                    if remove_files:
                        print("\n")
                        wsi_filelist = [f for f in wsi_filelist if f.name not in processed_files]
                        celldetector.logger.info(f"New processing amount: {len(wsi_filelist)}")
