...
def load_wsi_meta(
    ...
    if wsi_mpp is not None:
        ...
        try:
            ...
            if match:
                ...
                raise NotImplementedError("MPP must be defined either by metadata or by config file!")
        except:
            raise NotImplementedError("MPP must be defined either by metadata or by config file!")
    ...
    if wsi_magnification is not None:
        ...
        logger.warning("Based on resolution we assume x40 magnification. If not, please manually define magnification.")
        slide_mag = 40.0
    elif 0.40 <= slide_mpp <= 0.60:
        logger.warning("Based on resolution we assume x20 magnification. If not, please manually define magnification.")
        slide_mag = 20.0
    else:
        raise NotImplementedError("Magnification must be defined either by metadata or by config file!")
    ...
    if slide_mpp > 0.75:
        ...
        logger.error("An example for customized metadata is given in the examples.sh file")
        raise RuntimeError("See previous stacktrace, there is an error in the WSI MPP or metadata")
    ...
    if slide_mpp >= 0.20 and slide_mpp <= 0.30:
        ...
        logger.warning(f"We need to rescale to {target_mpp:<.5f}, handle with care! Check the final results!")

    slide_properties["slide_mpp"] = slide_properties["mpp"]  # to align with pathopatch...
