...
class WSI:
    ...
    def _get_metadata(self) -> None:
        ...
        with open(self.metadata_path.resolve(), "r") as metadata_yaml:
            try:
                ...
                print(exc)
        self.metadata["label_map_inverse"] = {v: k for k, v in self.metadata["label_map"].items()}
    ...
    def _get_wsi_patch_metadata(self) -> None:
        ...
        with open(self.patched_slide_path / "patch_metadata.json", "r") as json_file:
            ...
            self.all_patch_metadata = {str(list(elem.keys())[0]): elem[str(list(elem.keys())[0])] for elem in metadata}
    ...
    # patch processing
    def process_patch_image(self, patch_name: str, transform: Callable = None) -> Tuple[torch.Tensor, dict]:
    ...
    def get_patches(self, transform: Callable = None) -> Tuple[torch.Tensor, list, list]:
    ...
    def load_embedding(self) -> torch.Tensor:
        ...
        """
        embedding_path = self.patched_slide_path / "embeddings" / f"{self.embedding_name}.pt"
        if embedding_path.is_file():
            embedding = torch.load(embedding_path)
            return embedding
        else:
            raise FileNotFoundError(f"Embeddings for WSI {self.slide_path} cannot be found in path {embedding_path}")
...
class PatchedWSIInference(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, list[list[str, str]], list[str], int, str]:
        ...
        patch, metadata = self.wsi_object.process_patch_image(patch_name=patch_name, transform=self.transform)
