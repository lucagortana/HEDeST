...
def is_slurm() -> bool:
    ...
    """
    return any(var in os.environ for var in ["SLURM_JOB_ID", "SLURM_NODEID", "SLURM_CLUSTER_NAME"])
...
def is_kubernetes() -> bool:
    ...
    """
    return os.path.exists("/var/run/secrets/kubernetes.io/") or "KUBERNETES_SERVICE_HOST" in os.environ
...
def get_cpu_memory_slurm() -> Tuple[float, float]:
    ...
    if "SLURM_JOB_ID" in os.environ:
        try:
            ...
            mem_per_node_match = re.search(r"MinMemoryNode=(\d+)([KMGT])?", slurm_output)
...
def get_cpu_memory_kubernetes() -> Tuple[float, float]:
    ...
    if k8s_cpu_limit:
        ...
        if "m" in k8s_cpu_limit:
            cpu_count = float(k8s_cpu_limit.replace("m", "")) / 1000  # Convert "100m" to 0.1 CPU
        else:
    ...
    if k8s_mem_limit:
        ...
        if k8s_mem_limit.endswith("Ki"):
            ...
            try:
                memory_mb = float(k8s_mem_limit) / (1024 * 1024)  # Try to convert if it's a large number
            except ValueError:
                pass
            ...
            if os.path.exists(path):
                try:
                    with open(path, "r") as f:
                        ...
                        if memory_limit < 9223372036854000000:  # Ignore if it's effectively unlimited
    ...
    if cpu_count is None:
        cpu_count = len(os.sched_getaffinity(0)) if hasattr(os, "sched_getaffinity") else psutil.cpu_count(logical=True)
...
def get_cpu_memory_docker() -> Tuple[float, float]:
    ...
    if cpu_quota and cpu_period and cpu_quota > 0:
        ...
        cpu_count = len(os.sched_getaffinity(0)) if hasattr(os, "sched_getaffinity") else psutil.cpu_count()
    ...
    memory_mb = memory_limit / (1024 * 1024) if memory_limit else psutil.virtual_memory().total / (1024 * 1024)
    ...
    try:
        ...
        for i in range(gpu_resources["gpu_count"]):
            ...
            try:
                ...
                device_info["memory_free_gb"] = device_info["total_memory_gb"] - memory_allocated
            except:
        ...
        logger.info(f"CUDNN Version: {gpu_resources['details'].get('cudnn_version', 'Not available')}")
        ...
        if gpu_resources["gpu_count"] > 0:
            ...
            logger.info(f"CUDA Device 0 Total Memory: {device0['total_memory_gb']:.2f} GB")
        ...
        try:
            ...
            if slurm_output:
                if "K" in slurm_output:
                    return float(slurm_output.replace("K", "")) / 1024  # Convert KB to MB
                else:
                    try:
