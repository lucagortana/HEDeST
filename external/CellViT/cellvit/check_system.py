...
def main():
    ...
    print(f"\n{100*'*'}")
    print(f"* Error or warning messages here indicate that optional packages might not be installed correctly  *")
    print(f"* If an error occur, check if you installed to library accordingly                                 *")
    print(f"* CellViT++ will still work, but with limited features and might be severly slower                 *")
    print(f"{100*'*'}")
    ...
    if check_module("cupy"):
        ...
        logger.warning("CuPY cannot be loaded - falling back to numpy for postprocessing")
