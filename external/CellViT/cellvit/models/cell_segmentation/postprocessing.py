...
class DetectionCellPostProcessor:
    ...
    def _proc_np_hv(self, pred_inst: np.ndarray, object_size: int = 10, ksize: int = 21):
...
@jit(nopython=True)
def stack_pred_maps(nuclei_type_map: np.ndarray, nuclei_binary_map: np.ndarray, hv_map: np.ndarray) -> np.ndarray:
    ...
    pred_map = np.stack((nuclei_type_map, nuclei_binary_map, hv_map[..., 0], hv_map[..., 1]), axis=-1)
