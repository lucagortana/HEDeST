...
parser.add_argument(
    ...
    default="resnet50",
    choices=model_names,
    help="models architecture: " + " | ".join(model_names) + " (default: resnet50)",
)
parser.add_argument(
...
parser.add_argument(
    "--lr", "--learning-rate", default=0.1, type=float, metavar="LR", help="initial (base) learning rate", dest="lr"
)
...
parser.add_argument("-e", "--evaluate", dest="evaluate", action="store_true", help="evaluate models on validation set")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            print("=> loaded pre-trained models '{}'".format(args.pretrained))
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    it is not legitimate to change any part of the pre-trained models.
    BatchNorm in train mode may revise running mean/std (even if it receives
    no gradient), which are part of the models parameters too.
    """
    model.eval()
...
def sanity_check(state_dict, pretrained_weights, linear_keyword):
    ...
    for k in list(state_dict.keys()):
        ...
        # name in pretrained models
        k_pre = "module.base_encoder." + k[len("module.") :] if k.startswith("module.") else "module.base_encoder." + k
