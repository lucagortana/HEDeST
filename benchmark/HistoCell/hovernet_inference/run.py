...
class InferTile(object):
    ...
    def load_model(self):
        ...
        pred_config = PredictConfig(
            model=model_constructor(self.nr_types, self.patch_input_shape, self.patch_output_shape, self.input_norm),
    ...
    def process_all_files(self):
        ...
        pbar = tqdm.tqdm(desc="Processing Images", leave=True, total=len(file_list), ncols=80, ascii=True, position=0)
...
class InferWSI(object):
    ...
    def __run_inference(self, patch_top_left_list, tile_idx, nr_tiles):
        ...
        for patch_coord in patch_top_left_list:
            ...
            if self.factor_40_base > 1:
                ...
                win = cv2.resize(win, (self.patch_input_shape[1], self.patch_input_shape[0]), cv2.INTER_LINEAR)
        ...
        while len(sub_patches) > self.batch_size:
            ...
            batch_output_combined = list(zip(batch_info, batch_output))
            pred_list.extend(batch_output_combined)
            ###
            nr_proc += self.batch_size
            sys.stdout.write(
                "\rProcessing Batch (%d/%d) of Tile (%d/%d)" % (nr_proc, batch_count, tile_idx + 1, nr_tiles)
            )
            sys.stdout.flush()
        if len(sub_patches) != 0:
    ...
    def __gen_prediction(self, tile_info_list, patch_info_list):
        ...
        for idx in range(nr_tiles):
            ...
            selection = masking(patch_info_list[:, 0, 0, 0], start_coord[0], end_coord[0]) & masking(
                patch_info_list[:, 0, 0, 1], start_coord[1], end_coord[1]
            )
            ...
            if tile_patch_info_list.shape[0] == 0:
                assemble_and_flush(wsi_pred_map_mmap_path, tile_info, self.factor_40_base, None)
                continue
            ...
            patch_output_list = self.__run_inference(tile_patch_info_list[:, 0, 0], idx, nr_tiles)

            assemble_and_flush(wsi_pred_map_mmap_path, tile_info, self.factor_40_base, patch_output_list)
        return
    ...
    def __dispatch_post_processing(self, tile_info_list, callback):
        ...
        for idx in list(range(tile_info_list.shape[0])):
            ...
            if proc_pool is not None:
                ...
                results = post_proc_para_wrapper(wsi_pred_map_mmap_path, tile_info, func_kwargs)
                callback(results)
        if proc_pool is not None:
            proc_pool.close()
            proc_pool.join()
    ...
    def load_wsi(self, filename):
        ...
        self.wsi_ds_lvl = self.wsi_handler.metadata["level_downsamples"][self.wsi_proc_lvl]
        self.ds_factor_mask = self.wsi_handler.metadata["magnification"][self.wsi_proc_lvl] / 1.25
    ...
    def process_wsi(self, filename):
        ...
        self.patch_input_shape_tmp = (patch_input_shape_tmp / self.factor_40_base).astype("int")
        ...
        inf_tile_input_shape = np.array(self.inf_tile_shape) / self.factor_40_base
        ...
        def postproc_fixborder_callback(args):
            ...
            roi_inst = self.wsi_inst_map[tile_tl[0] : tile_br[0], tile_tl[1] : tile_br[1]]
            roi_inst = np.copy(roi_inst)
            roi_edge = np.concatenate([roi_inst[[0, -1], :].flatten(), roi_inst[:, [0, -1]].flatten()])
            roi_boundary_inst_list = np.unique(roi_edge)[1:]  # exclude background
            roi_inner_inst_list = np.unique(roi_inst)[1:]
            roi_inner_inst_list = np.setdiff1d(roi_inner_inst_list, roi_boundary_inst_list, assume_unique=True)
            roi_inst = remove_inst(roi_inst, roi_inner_inst_list)
            self.wsi_inst_map[tile_tl[0] : tile_br[0], tile_tl[1] : tile_br[1]] = roi_inst
            ...
            inner_inst_list = np.setdiff1d(inner_inst_list, boundary_inst_list, assume_unique=True)
        ...
        proc_tile_coords = get_tile_info(self.wsi_proc_shape, proc_tile_shape, self.ambiguous_size)
        ...
        pbar_creator = lambda x, y: tqdm.tqdm(desc=y, leave=True, total=int(len(x)), ncols=80, ascii=True, position=0)
        ...
        for inst_id, inst_info in self.wsi_inst_info.items():
            ...
            for info_name, info_value in inst_info.items():
                ...
                new_inst_info[info_name] = info_value
            json_dict[int(inst_id)] = new_inst_info
        with open("%s/nuclei_dict.json" % self.output_dir_wsi, "w") as handle:
            json.dump(json_dict, handle)
