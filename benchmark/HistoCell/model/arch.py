...
class HistoCell(nn.Module):
    def __init__(self, config) -> None:
        ...
        if config.channels == 1:
            modules = [
                nn.Conv2d(config.channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        ...
        self.size_embed = nn.Sequential(nn.Linear(2, 16), nn.ReLU())
        self.merge = nn.Linear(512 + 16, 512)

        self.out = nn.LSTM(input_size=512, hidden_size=512, num_layers=1, batch_first=True)
        self.predict = nn.Sequential(nn.Linear(512, config.k_class), nn.Softmax(dim=-1))
...
class HistoState(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.out = nn.LSTM(input_size=512, hidden_size=512, num_layers=1, batch_first=True)
        self.predict1 = nn.Sequential(nn.Linear(512, config.k_class), nn.Softmax(dim=-1))
        self.predict2 = nn.Sequential(nn.Linear(512, config.k_state), nn.Softmax(dim=-1))
        self.tc = nn.Linear(512, config.tissue_class + 1)
