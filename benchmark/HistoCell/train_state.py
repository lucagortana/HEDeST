...
def main():
    ...
    print(data_prefix)
    train_data = TileBatchStateDataset(
    ...
    train_loader = DataLoader(
        train_data, batch_size=config.data.batch_size, shuffle=True, num_workers=6, pin_memory=True
    )
...
def train_loop(epoch, model, train_loader, loss_func, aux_loss, opt, device, with_state=False):
    ...
    for idx, data in enumerate(train_bar):
        ...
        images, cell_proportion, state_proportion = (
            ...
            data["cells"].to(torch.float32).to(device),
            data["states"].to(torch.float32).to(device),
        )
        ...
        loss_out = (
            loss_func((cell_proportion + 1e-10).log(), pred_dict["type_prop"] + 1e-10)
        ...
        if with_state:
            ...
            state_loss = loss_func((state_proportion + 1e-10).log(), pred_dict["state_prop"] + 1e-10) + loss_func(
                (pred_dict["state_prop"] + 1e-10).log(), state_proportion + 1e-10
            )
